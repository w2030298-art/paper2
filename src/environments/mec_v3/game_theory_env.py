...
class HierarchicalReward:
    """Reward = alpha * immediate + beta * cooperative + gamma * equilibrium."""
    ...
    def compute(
        ...
        r_imm = (
            -0.4 * latency / max(latency_budget, EPS)
            -0.3 * energy / max(energy_budget, EPS)
            -0.3 * (max(0.0, latency - latency_budget) ** 2) / max(latency_budget * latency_budget, EPS)
        )
        r_coop = float(shapley_value) * float(cooperation_gain)
...
class GameTheoryMECEnv(BaseMECEnv):
    ...
    def _coalition_value(self, coalition: frozenset[int]) -> float:
        ...
        score = 0.0
        for i in coalition:
            best_snr = float(np.max(self.channel_qualities[i]))
            score += max(best_snr, -10.0) + 10.0
        queue_penalty = float(np.mean(self.queue_lengths))
        return max(score - queue_penalty, 0.0)

    def _compute_shapley_allocation(self) -> np.ndarray:
        if not self.enable_shapley:
            return np.full(self.num_agents, 1.0 / max(self.num_agents, 1), dtype=np.float32)

        raw_phi = self.shapley_estimator.compute(self._coalition_value)
        # Keep positive and normalized for reward allocation.
        raw_phi = raw_phi - np.min(raw_phi) + 1e-6
        s = float(np.sum(raw_phi))
        if s <= 0:
            phi = np.full(self.num_agents, 1.0 / max(self.num_agents, 1), dtype=np.float32)
        else:
            phi = (raw_phi / s).astype(np.float32)
    ...
    def _process_action_game_theory(
        ...
        if target == 0 or off <= 1e-3:
            return float(local_time), float(local_energy)
    ...
    def step(
        ...
        individual_offload: List[float] = []
        for i in range(self.num_agents):
            latency, energy = self._process_action_game_theory(
        ...
        # Team/cooperation signals.
        team_value = -(
            np.mean(individual_latencies) / max(self.latency_budget, EPS)
            + 0.2 * np.mean(individual_energies) / max(self.energy_budget, EPS)
        )
        indiv_values = np.array(
            [
                -(individual_latencies[i] / max(self.latency_budget, EPS) + 0.2 * individual_energies[i] / max(self.energy_budget, EPS))
                for i in range(self.num_agents)
            ],
            dtype=np.float32,
        )
        cooperation_gain = float(team_value - np.mean(indiv_values))
        ...
        for i in range(self.num_agents):
            ...
            r, terms = self.reward_model.compute(
                ...
                shapley_value=float(shapley_alloc[i]),

...
- 基线：
  ...
  - 分支：`main`
- 变更类型：`patch`
- 本轮目标：在已完成的收敛质量管线基础上，补齐未收敛算法 failure matrix、异常事件诊断脚本与稳定化候选配置。
- 总模块数：12
- 既有步骤数：51（模块 1-9，作为当前仓库基线锁定）
- 本轮新增步骤数：6
- 预计步骤总数：71
- 建议开发顺序：先执行上轮提交卫生 fix，再执行模块 12 Step 1 → Step 6
- 创建日期：2026-05-02
- 最后更新：2026-05-02
...
| v4 | 2026-05-02 | 追加模块 12：未收敛算法诊断、failure matrix 与稳定化候选配置；统一归档目录为 `docs/archive/` |
...
- 当前阶段：模块 12 Step 6 完成
- 整体进度：71 / 71 步骤完成（模块 1-12 已完成）
- 状态：completed；模块 12 的 review scope 步骤待审核
- 阻塞项：
  - 无。
...
无。本轮不直接重跑 full 17，不启用稳定化覆盖配置；先用诊断脚本和 failure matrix 判断极端负 reward 是否为异常事件。
...
- 职责：对上轮 full 17 中未形成可信收敛趋势的算法生成 failure matrix，区分坏平台、震荡、发散和灾难性异常点。
- 不做事项：
  - 不直接重跑 full 17。
  - 不把 IQL/VDN/IPPO 的极端负 reward 当成普通调参问题掩盖。
  - 不默认启用稳定化覆盖配置。
  - 不修改 reward 公式、环境物理模型或算法网络结构。
- 目标算法：GRPO、SAC、A3C、TRPO、MAPPO、COMA、IPPO、VDN、MADDPG、IQL、MATD3。
...
- 范围：
  ...
  - 统一 `docs/archive/` 与 `docs/.archive/`，后续只使用 `docs/archive/`。
...
- 功能：
  - 读取 `results/benchmark*.json` 或显式 `--input`。
  - 读取 `figures/convergence_quality_report.json` 或显式 `--quality-report`。
  - 可选读取 `experiments/<run_id>/artifacts/<ALGO>/checkpoints/train_logs.json`。
  - 输出 `results/convergence_failure_matrix.json` 与 `docs/convergence_failure_analysis.md`。
- 诊断指标：
  ...
  - `failed_seed_count`
  - `plateau_badness`
  - `reward_regression_from_initial`
  - `reward_regression_from_best`
- 验证：
  ```bash
...
- 要求：
  - 作为候选配置保存，不在 benchmark 默认路径中直接启用。
  - 分组包含 `on_policy_high_variance`、`continuous_actor_critic`、`value_decomposition`、`conservative_policy_update`。
  - 候选动作覆盖降低学习率、gradient clipping、exploration/entropy 调整、replay warmup、target update、eval episodes。
- 验证：
  ```bash
  python -c "import yaml; yaml.safe_load(open('configs/stability_overrides.yaml', encoding='utf-8'))"
  ```

## Step 5：先做 targeted rerun 计划，不重跑 full 17

- **scope: review**
- 目标算法：GRPO、SAC、A3C、TRPO、MAPPO、COMA、IPPO、VDN、MADDPG、IQL、MATD3。
- 运行策略：
  - 第一轮每算法 50k steps，确认不再出现明显异常。
  - 第二轮每算法 100k 或 200k steps，确认趋势。
  - 最后才考虑完整 17 算法重跑。
- 输出规划：
...
```bash
pytest tests/test_convergence_plot.py tests/test_docs_contract.py tests/test_analyze_convergence_failures.py -q
```
...
python -c "import scripts.benchmark; print('OK')"
python -c "from scripts.analyze_convergence_failures import build_failure_matrix; print('OK')"
```
...
7. 模块 12 只做诊断脚本、failure matrix 和候选配置，不直接启用调参覆盖，也不重跑 full 17。

...
> 版本：v2.0 — 2026-04-24
>
> 修订说明：就第一版稿件进行了全面的符号一致性核查、公式推导勘误、逻辑链条补全以及学术表述润色。具体修订要点列于文末附录C。
...
在仿真实现中设置利用率上限 $\rho_k^{\max} = 0.95$ 以避免数值发散。
...
$$\mathbf{u}_i(t+1) = \mathbf{u}_i(t) + \mathbf{v}_i(t) \cdot \Delta t$$

**边界约束。** 当用户移动至部署区域边界时，采用弹性反射策略将位置限制在 $[\mathbf{u}_{\min}, \mathbf{u}_{\max}]$ 内。

**初始分布。** 用户初始位置在极坐标下随机采样：$\theta_i^{(0)} \sim \mathcal{U}[0, 2\pi)$，$r_i^{(0)} \sim \mathcal{U}[r_{\min}, r_{\max}]$（$r_{\min} = 8$ m，$r_{\max} = 30$ m），最大速度 $v_{\max} = 5$ m/s（对应步行至慢跑场景）。
...
其中 $\sigma(\cdot)$ 为 Sigmoid 函数，乘以系数 5 以使输出在 $[-1, 1]$ 输入范围内覆盖 $[0.007, 0.993]$ 的有效区间。

**第二层：约束投影。** 对于能量预算约束 C4，若估计能耗 $\hat{E}_i > E_i^{\max}$，按比例缩放频率与功率：$\text{scale} = \sqrt{E_i^{\max}/\hat{E}_i}$，$f_i \leftarrow f_i \cdot \text{scale}$，$P_i \leftarrow P_i \cdot \text{scale}$。功率约束 C6 类似处理。
...
其中 $v_E = \max(0, \hat{E}_i - E_i^{\max})$ 等为约束违反量，$\text{slack}_c$ 为约束松弛量，$\mu_{\text{pen}} = 0.05$，$\mu_{\text{bar}} = 10^{-4}$。
...
式 (6) 含隐式超越函数，通过牛顿法迭代求解：$p_k^{(n+1)} = p_k^{(n)} - \Pi_k'(p_k^{(n)}) / \Pi_k''(p_k^{(n)})$，初始化 $p_k^{(0)} = (p_{\min} + p_{\max})/2$，收敛准则 $|\Pi_k'| < 10^{-6}$。
...
用户 $i$ 的 Shapley 值为其对所有可能联盟的平均边际贡献：
...
式 (8) 满足效率性 $(\sum_i \phi_i = v(\mathcal{U}))$、对称性、零元素性与可加性四条 Shapley 公理。

#### III-D-3. 蒙特卡洛-对偶近似算法

精确计算式 (8) 的复杂度为 $O(2^U)$，在用户数 $U \geq 10$ 时不可行。本文采用基于排列采样的蒙特卡洛方法，并通过对偶变量（Antithetic Variates）技术将方差缩减约一半。
...
**复杂度：** $O(M \cdot U \cdot T_v)$，其中 $T_v$ 为联盟值一次评估时间。方差界 $\text{Var}(\hat{\phi}_i) \leq V_{\max}^2 / (2M)$（对偶采样使有效样本量翻倍）。
...
为使 EFX 估值函数反映用户的异质偏好，引入条件偏好网络（CP-nets）。CP-net 为一有向无环图，每个节点代表一个偏好属性，边表示条件依赖。本文中偏好结构由任务紧急度条件驱动：
...
- $\mathcal{S}$：全局状态空间，包含所有基站队列 $\{Q_k\}$、信道状态 $\{g_{i,k}\}$、用户位置 $\{\mathbf{u}_i\}$ 和任务参数 $\{\mathcal{J}_i\}$
- $\mathcal{O}_i \subset \mathcal{S}$：智能体 $i$ 的局部可观测子空间（部分可观测性源于用户无法直接获知其他用户的策略）
- $\mathcal{A}_i$：混合动作空间（离散 + 连续），详见 IV-C 节
- $P(s' | s, \mathbf{a})$：由通信模型（I-B）、计算模型（I-C）和移动性模型（I-D）共同决定的状态转移核
- $R_i : \mathcal{S} \times \prod_j \mathcal{A}_j \to \mathbb{R}$：分层奖励函数，详见 IV-D 节
- $\gamma = 0.99$：折扣因子
...
各分量含义：$\tilde{Q}_k = Q_k/Q_{\max}$，$\tilde{\Gamma}_{i,k}$ 为归一化后的 SINR，$L_k = \rho_k$ 为 CPU 利用率，$e_i$ 为归一化剩余能量，$\tau_i = T_i^{\max}/T_{\max}^{\max}$ 为归一化截止时间，$\tilde{D}_i, \tilde{C}_i$ 为归一化任务参数，$\tilde{v}_i = v_i/v_{\max}$ 为归一化速度。

全局状态（用于中心化 Critic 训练）：$s(t) = [o_1(t), \ldots, o_U(t),\, \mathbf{p}^*(t),\, \mathbf{a}^*_{\text{eq}}(t)]$，其中 $\mathbf{a}^*_{\text{eq}}$ 为 Stackelberg 均衡动作提示。

### IV-C. 混合动作空间

$$a_i = \bigl\{\text{target}: x_i \in \{0, 1, \ldots, K\},\;\; \text{ratio}: (r_\alpha,\, r_f,\, r_P) \in [-1, 1]^3\bigr\}$$

离散头输出卸载目标，连续头经 II-C 节映射后得到物理决策变量 $(\alpha_i, f_i^{\text{loc}}, P_i)$。双头 Actor 网络在共享特征层后分叉：离散头接 Softmax 输出分类概率，连续头接 Tanh 激活限制到 $[-1, 1]$。

### IV-D. 分层奖励函数

奖励函数由三个语义明确的层次构成。

**即时通信性能奖励 $r_{\text{imm}}$。** 直接反映物理层性能：
...
其中 $(\omega_T, \omega_Q, \omega_D, \omega_E) = (0.55, 0.15, 0.20, 0.10)$，$\delta_{\text{nn}} = 0.02 \cdot \mathbb{1}[\text{非最近基站}]$ 为空间合理性引导项。

**协作奖励 $r_{\text{coop}}$。** 基于 Shapley 值量化个体对团队的贡献：

$$r_{\text{coop}} = \text{clip}\!\left(\phi_i \cdot \left[\bar{J}^{\text{baseline}} - \bar{J}^{\text{observed}}\right],\; -1,\; 1\right)$$

其中 $\bar{J}^{\text{baseline}}$ 为全用户纯本地策略的平均通信代价，$\bar{J}^{\text{observed}}$ 为当前联合策略的平均通信代价。$r_{\text{coop}} > 0$ 当且仅当当前联合策略优于独立基线。

**均衡一致性奖励 $r_{\text{eq}}$。** 引导策略靠近博弈均衡解：

$$r_{\text{eq}} = -\frac{\|\mathbf{a}_i - \mathbf{a}_i^*(s)\|_2^2}{\dim(\mathcal{A})}$$

其中 $\mathbf{a}_i^*(s)$ 为双层博弈求解器给出的均衡动作。
...
负相关（冲突）时自动降权，正相关（协同）时保持或提升。更新后重新归一化使权重和为 1。
...
**中心化 Critic（训练阶段）：** 以全局状态为输入，附加均衡嵌入模块 $\text{EqEnc}: \mathbb{R}^{U \cdot \dim(\mathcal{A})} \to \mathbb{R}^{64}$：
...
$$\mathcal{L}^{\text{GRPO}}(\theta_i) = -\mathbb{E}\!\left[\min\!\left(\frac{\pi_{\theta_i}(a_i|o_i)}{\pi_{\theta_i^{\text{old}}}(a_i|o_i)} \hat{A}_i,\;\; \text{clip}\!\left(\frac{\pi_{\theta_i}}{\pi_{\theta_i^{\text{old}}}},\, 1\!-\!\epsilon,\, 1\!+\!\epsilon\right)\hat{A}_i\right)\right] - c_{\text{ent}} H[\pi_{\theta_i}]$$

其中 $\epsilon = 0.2$，$c_{\text{ent}} = 0.01$。

#### IV-E-3. Shapley 值信用分配

团队总奖励通过 Shapley 值进行信用分配：

$$r_i^{\text{credit}} = \begin{cases} \phi_i \cdot R^{\text{team}} \big/ \sum_j \phi_j, & \text{if } |\sum_j \phi_j| > 10^{-8} \\ R^{\text{team}} / U, & \text{otherwise} \end{cases}$$
...
即策略动作以 $O(1/\sqrt{T})$ 的速率收敛至 Stackelberg 均衡的 $\varepsilon$-邻域。
...
| $\epsilon$ (clip) | 0.2 | Rollout steps | 2048 |
| $c_{\text{ent}}$ | 0.01 | Update epochs | 10 |
| $G$ (组大小) | 4 | Max grad norm | 0.5 |
| $\eta$ (学习率) | $3\times10^{-4}$ | 总训练步数 | 500,000 |
| $H$ (隐藏维度) | 256 | $N_{\text{warm}}$ | 1,000 |
| $(\omega_\alpha, \omega_\beta, \omega_\gamma)$ | (0.8, 0.1, 0.1) | 探索 $\epsilon$: 初始/终止 | 1.0 / 0.05 |
...
**基线算法：** 启发式（Greedy, Random, Local-only, Full-offload）、单智能体DRL（DQN, DDPG, TD3, SAC）、多智能体DRL（QMIX, MAPPO w/o game, MADDPG）。

**消融实验：** A1) w/o Shapley → 均匀分配；A2) w/o Warm-Start → 随机初始化；A3) w/o 移动性 → 静态用户；A4) w/o $r_{\text{coop}}$ → 仅即时奖励；A5) w/o M/M/1 → 无排队时延；A6) w/o $r_{\text{eq}}$ → 无均衡引导；A7) w/o DVFS → 理想 $\kappa f^2 C$ 模型。
...
| 类型 | 算法 |
|------|------|
| 单智能体 On-Policy | PPO, TRPO, A3C |
| 单智能体 Off-Policy | DDQN, DDPG, TD3, SAC |
| 多智能体值分解 | QMIX, VDN, IQL |
| 多智能体策略梯度 | MAPPO, IPPO, MADDPG, MATD3, COMA |
| 本文算法 | GRPO + Stackelberg + Shapley |
| 偏好对齐扩展 | SimPO |
...
| R13 | 在符号表中新增"易混符号辨析"列 | 统一解决 8.1 节符号一致性检查清单指出的全部冲突 |
